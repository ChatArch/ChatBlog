...
description: 介绍 ChatArch 为什么把 Mattermost 作为自托管实时 Agent 工作间，当前 matter.public.wzhecnu.cn 入口验证了什么，以及 Hermes 如何通过 Mattermost Gateway 进入频道、线程和团队协作流。
unlisted: true
ai_slop: true
ai_slop_reason: "篇幅围绕内部社区愿景，真实验收只有健康接口，核心 Agent 接入仍是未来计划。"
---
...
:::info 证据口径
本文使用三类来源：一是 `matter.public.wzhecnu.cn` 的公网回读和入口验证；二是 Hermes 官方 Mattermost 文档（[Mattermost | Hermes Agent](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/mattermost/)）；三是 ChatArch 前面关于 Hermes 平台入口、ChatRSS TriggerEvent 和 Agent Community 的实践文章。文中只写公开 URL、公开 API health、产品架构和非敏感配置项，不写凭据、密码或私有运维细节。
:::

...
description: 调研千问 AI 平台 Token Plan 的个人版、团队版、Credits 额度、模型清单、Codex/CRS/Hermes/Claude Code 等多渠道配置，以及 OpenAI-compatible、Responses 和 Anthropic 兼容格式的选择边界。
unlisted: true
ai_slop: true
ai_slop_reason: "前称个人版不得作后端/API 池，后又建议共享池，边界矛盾；OK 烟测也不足以支撑模型能力推荐。"
---
...
- [Hermes Agent 接入文档](https://platform.qianwenai.com/docs/developer-guides/clients-and-developer-tools/hermes-agent)
- [Claude Code 接入文档](https://platform.qianwenai.com/docs/developer-guides/clients-and-developer-tools/claude-code)
- [OpenAI 兼容接口](https://platform.qianwenai.com/docs/api-reference/toolkitframework/openai-compatible/overview)、[OpenAI Responses](https://platform.qianwenai.com/docs/api-reference/chat/openai-responses) 与 [Anthropic 兼容 API](https://platform.qianwenai.com/docs/api-reference/chat/anthropic)
...
1. **OpenAI 兼容接口**：文档明确说已有 OpenAI SDK/REST 代码时，只需改 `base_url`、`api_key` 和 `model` 三个参数即可迁移到千问。
2. **OpenAI Responses API**：`/compatible-mode/v1/responses` 是一等 API；Codex 文档也明确让支持 Responses 的模型用最新版 Codex 和 `wire_api = "responses"`。
3. **Anthropic 兼容 API**：`/apps/anthropic/v1/messages` 也存在；Claude Code 文档就要求设置 `ANTHROPIC_BASE_URL`、`ANTHROPIC_AUTH_TOKEN` 和 `ANTHROPIC_MODEL`。

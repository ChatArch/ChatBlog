...
:::info 证据口径
快照时间为 **2026-08-06 CST**。证据来自千问 AI 平台公开定价与文档、一次脱敏的 `/models` 只读发现、一次 Codex 最小 smoke，以及一台已有 CRS 实例的只读代码/运行态检查。本文不包含任何 API Key、账号密钥、服务器地址或私有路径。
:::
...
| `accountType` | 先 `dedicated` 测试，稳定后再决定是否 `shared` |
| `isActive` / `schedulable` | 先 false / false 建账校验，再启用 |
...
## 现有 CRS 生态的缺口：服务支持，运维 CLI 还不完整

只读检查显示，已有 CRS 服务本身支持 `OpenAI-Responses` 账号，也有 Admin API：
...
但当前安装的 ChatCRS 运维 CLI 仍停留在健康检查、调试、verify、cutover 等命令面，没有稳定的：
...
1. **CRS runtime 层：可接入。** 千问 Token Plan 能作为 OpenAI-Responses API-key 账号新增。
2. **ChatCRS 运维生态层：建议补 CLI。** 否则新增账号仍要手动走 Web/Admin API，不够 ChatArch-contained，也不够可审计。
...
2. **CRS 接入实践 PR/任务**：用轮换后的专用 Key 在非共享路径上创建账号，绑定临时 CRS Key，跑最小 Responses smoke，再决定是否启用共享池。

只有这两步都完成，才算真正把千问 Token Plan 接进 ChatArch 的 CRS 生态，而不是只在某台机器上手工保存了一把 Key。

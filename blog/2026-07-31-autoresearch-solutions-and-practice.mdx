---
title: "AutoResearch 怎么落地：行动空间、验证债务与可信研究闭环"
...
# AutoResearch 怎么落地：行动空间、验证债务与可信研究闭环
...
| 循环 | 被更新的状态 | 系统动作 | 新观测 | 决策者 |
|---|---|---|---|---|
| **执行循环** | 代码、配置、环境、候选程序 | patch、运行、调参、回退 | metric、错误、资源消耗 | 固定 evaluator 与 guardrail |
| **探究循环** | 竞争假设、已有证据、实验设计 | 选择实验、采样、改变条件 | 数据、效应、反例、不确定性 | 预先声明的统计/因果规则与研究者 |
| **问责循环** | claim-evidence graph、复现状态、发布状态 | 审计、复验、缩小主张、补实验 | 独立 review、复现结果、证据缺口 | 与 proposer 分离的 reviewer / human gate |
...
每个候选在同一台 GPU 上训练固定 5 分钟，`val_bpb` 攭善就保留，否则回退。官方进展图记录了一次 83 个候选、15 次保留改进的运行：

![autoresearch 官方实验进展图](/img/autoresearch-guide/autoresearch-progress.png)

图源：[karpathy/autoresearch `progress.png`](https://github.com/karpathy/autoresearch/blob/master/progress.png)。
...
[AutoRA](https://github.com/AutoResearch/autora)采用更接近经验科学的 state loop：experimentalist 基于当前状态选择条件，experiment runner 产生观测，theorist 根据累计数据更新模型，再把新状态交给下一轮。

![AutoRA 官方经验科学闭环](/img/autoresearch-guide/autora-empirical-loop.png)

图源：[AutoResearch/autora `overview.png`](https://github.com/AutoResearch/autora/blob/main/docs/img/overview.png)。
...
```yaml
question: "Does the proposed mechanism improve generalization without increasing latency?"

hypotheses:
  primary: "The mechanism reduces validation error by improving representation quality."
  rival_explanations:
...
| 现有条件 | 建议起点 | 它解决什么 | 首先偿还的验证债务 |
|---|---|---|---|
| 一个 ML repo、明确 metric、便宜实验 | [autoresearch](https://github.com/karpathy/autoresearch) | 最小可执行搜索合同 | 独立 confirmation、query budget、guardrail |
| 同时保留多条代码路线 | [AIDE](https://github.com/WecoAI/aideml) | solution tree 与 debug/improve | 多重比较、winner's curse |
| 已有量化/Kaggle/模型领域 runner | [RD-Agent](https://github.com/microsoft/RD-Agent) | idea 与 implementation 分工 | idea、patch、run、feedback 分别留痕 |
| 下一次数据采集本身需要选择 | [AutoRA](https://github.com/AutoResearch/autora) / [Curie](https://github.com/Just-Curieous/Curie) | 主动实验与状态更新 | measurement bias、证伪规则、人工执行 gate |
| 已有稳定实验平台，研究端到端编排 | [AI Scientist v1/v2](https://github.com/SakanaAI/AI-Scientist-v2) | idea、实验树、写作与 review 协议 | sandbox、文献遗漏、自我 review、独立确认 |
| 结构化数据需要论文级追溯 | [data-to-paper](https://github.com/Technion-Kishony-lab/data-to-paper) | 数字到代码的 evidence chain | 统计方法有效性与领域专家审查 |
| 多天任务需要恢复、接管和审计 | [DeepScientist](https://github.com/ResearAI/DeepScientist) / [ARIS](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep) | Research OS 与 workflow control | 另建 evaluator；持久状态不能替代科学确认 |

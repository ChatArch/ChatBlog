...
flowchart TB
  ...
  P --> X[Isolated worktree and runner]
  X --> E[Exploration evaluator and ledger]
  E -->|iterate within query budget| P
  E --> S[Frozen shortlist]
  S --> C[Locked confirmation]
  C --> F[Fresh data, seeds, decision rule]
  F --> A[Evidence store]
  A --> R[Independent reviewer]
  R --> G{Human release gate}
  G -->|revise or reject| H
  G -->|accept| O[Report or paper]
```

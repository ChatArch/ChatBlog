...
unlisted: true
ai_slop: true
ai_slop_reason: "时效性选会清单停留在7月快照，部分主推窗口已过；保留作历史资料，不作当前推荐。"
---
...
2. **当前节点是新投稿还是修改后重投。** “only for major revisions” 不能算普通新稿机会。
3. **投稿类别（track）是什么。** Main research、short、industry、findings 和 companion 的篇幅、评价标准与出版位置可能不同。
4. **时区到底是什么。** AoE / UTC-12 的 `23:59`，在北京时间通常是次日 `19:59`。

本文后续以**会议届次**作为主口径：同一会议的不同年份分别计数。CCFDDL 在补充窗口内给出 **40 个 CCF A/B 会议届次**；官网补充的 CAV 2027 再增加一届，因此广筛地图共 **41 个会议届次**。

<details>
<summary>完整投稿入口计数</summary>

同一会议届次可能有多个投稿轮次（round）或投稿类别（track），所以入口数不能与会议数直接相加。CCFDDL 中，剩余 2026 年有 46 个开放入口；延长到 2027 年 1 月后有 51 个。再加入 SANER Agentic AI4SE、CAV 2027 和 FSE Industry Papers 三个官网补充入口后，共 54 个。
...
| Fit-S | [AAMAS 2027 Main Track](https://warwick.ac.uk/fac/sci/dcs/aamas2027/calls/)（CCF-B） | 10 月 2 日 19:59 | 10 月 9 日 19:59 | planner–prover–critic、多 Agent 协作、工具调用与 Agent 验证 |
...
两者都接受 Agent，但 AAMAS 关心 Agent 与多 Agent 方法本身，SANER 关心 Agent 是否解决真实软件维护问题。
...
数学 tutor、科学助手、交互式证明工具看起来适合 IUI 或 CHI，但两个会议都不会因为有聊天界面就自动接受。
...
第一轮广筛没有把数据、系统和计算会议删掉。以会议届次计，40 个 CCFDDL 候选中有 20 个属于 Fit-B：
...
同样以会议届次计，Fit-C 共有 11 个：USENIX Security、NDSS、MobiCom、NSDI、DATE、EUROCRYPT、DCC、ISCAS、CHES、IEEE S&P、SECON。
...
1. **入口**：摘要、注册和全文 deadline 是否都还没过？
2. **投稿类别**：main、short、industry、findings、journal-first、companion，还是只接受修改稿或重投？
...
只按会议名字筛，会漏掉 WSDM 的 AI for Data-rich Science、SANER 的 Agentic AI4SE、FSE 的 proof engineering 和 CAV 的 ML verification；只按日期筛，又会把摘要已过、仅限修改后重投或投稿类别完全不同的入口混在一起。

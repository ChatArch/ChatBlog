...
- 本轮成功结果都由外层记录脚本写入文章状态台账，而不是只留在终端输出里。
...
我们在它外层增加一个很薄的 wrapper：先计算 source hash，调用 CLI，拿到成功 receipt 后再写入文章台账。上游 CLI 当前没有内置这层持久状态，不能把终端中的成功链接误当成已经具备幂等控制。
